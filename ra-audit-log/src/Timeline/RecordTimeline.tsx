...
import { useRecordEvents, UseRecordEventsOptions } from '../dataProvider';
 ...
 * }
 */
export const RecordTimeline = (props: RecordTimelineProps): ReactElement => {
    const { loaded, data, ids } = useRecordEvents(props);
    const { page, perPage, sort, order, record, resource, ...rest } = props;

    return (
        <Timeline loaded={loaded} records={ids.map(id => data[id])} {...rest}>
            <TimelineGroup>
                <RecordTimelineItem />
            </TimelineGroup>
        </Timeline>
...
export type RecordTimelineProps = UseRecordEventsOptions &
    Omit<TimelineProps, 'records' | 'loaded'>;
