...
var readStorage = function (key) {
    var preferences = JSON.parse(getStorage().getItem(RA_PREFERENCE_WORKSPACE)) || {};
    if (key) {
...
var writeStorage = function (key, value) {
    ...
    try {
        ...
        var newPreferences = key && value != undefined
            ? set(__assign({}, preferences), key, value)
            : key || value;
...
var usePreferences = function (key, defaultValue) {
    ...
    var _a = useState(initialStoredValue != undefined ? initialStoredValue : defaultValue), localState = _a[0], updateLocalState = _a[1];
    ...
    var onLocalStorageChange = function (event) {
        ...
        var newValue = get(event.newValue, key /* my own key */);
        // and we update our state no matter what. If same value, it won't redraw anyway
        updateLocalState(newValue);
    };
    ...
    useEffect(function () {
        ...
        var canWrite = getStorage().getItem(RA_PREFERENCE_WORKSPACE) === null;
    ...
    var writeState = useCallback(function (value) { return writeStorage(key, value); }, [
        key,
    ]);
