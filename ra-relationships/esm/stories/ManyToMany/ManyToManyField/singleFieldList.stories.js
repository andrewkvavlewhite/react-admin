...
export default {
    title: 'ra-relationships/many-to-many/ReferenceManyToManyField/SingleFieldList',
...
export var LoadedWithData = function () { return (React.createElement(TestContext, { initialState: initialState },
    React.createElement(ReferenceManyToManyField, { label: "performances", resource: text('resource', 'bands'), source: text('source', 'id'), reference: text('reference', 'venues'), through: text('through', 'performances'), using: text('using', 'band_id,venue_id'), record: data.bands[0] },
