...
var useReferenceManyToManyFieldController = function (_a) {
    ...
    var _e = using.match(constants_1.UsingRegexp), sourceField = _e[1], targetField = _e[2];
    // TODO: Introduce real pagination (setters, etc.)
    var page = 1;
    var throughManyReferences = react_admin_1.useGetManyReference(through, sourceField, get_1.default(record, source), { page: page, perPage: perPage }, sort, filter, resource);
    ...
    var referencesIds = unique
        ? uniq_1.default(allReferencesIds)
        : allReferencesIds;
    var references = react_admin_1.useGetMany(reference, referencesIds);
    // Memoize the references data and only recompute them when they are reloaded
    var referencesData = react_1.useMemo(function () {
...
exports.default = useReferenceManyToManyFieldController;
